...
class Puzzle
  attr_reader :data, :numbers, :max_magnitude
  ...
  def perform
    ...
    @numbers.each do |number_1|
      @numbers.each do |number_2|
        local_number = SailfishNumber.new([number_1.to_a, number_2.to_a])
        local_number.simplify

        local_magnitude = local_number.magnitude
        @max_magnitude = local_magnitude if @max_magnitude < local_magnitude
      end
    end
