...
class Puzzle
  attr_reader :data, :grids, :drawn_numbers, :last_grid, :current_number
  ...
  def perform
    ...
    while @grids.any?
      ...
      @last_grid = @grids.find(nil, &:valid?)
      @grids = @grids.reject(&:valid?)
    end
  ...
  def to_s
    <<~OUTPUT
      Result: #{@last_grid.result * @current_number}
    OUTPUT
  end
end
