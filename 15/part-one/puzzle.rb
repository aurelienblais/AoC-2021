...
class Graph
  ...
  def add_node(value, x, y)
    @nodes << Node.new(value, x, y)
  end
  ...
  def initialize(file:)
    ...
    @data.each_with_index do |row, y|
      row.split('').each_with_index do |value, x|
        @graph.add_node value.to_i, x, y
      end
    end
  end

  def perform
    @result = @graph.dijkstra(@graph.find_node(0, 0), @graph.find_node(@data[0].length - 1, @data.length - 1))
